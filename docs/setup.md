...
## V1 voice simulator setup

**Incoming supervised-demo calls:** Enable `VOICE_INBOUND_ENABLED=true` with a separate random 64-hex-character `VOICE_INBOUND_KEY`. In Twilio's trial Inbound → Custom settings for the enrolled handset, save the generated `TwilioProvider.inboundUrl()` using POST. The bootstrap URL expires after one hour and must be refreshed for a later session; it is a bearer credential and must not be logged or shared. Incoming requests must identify CallSid and From; before creating the session, the adapter reads Twilio's call record and requires this account, the enrolled caller, the configured trial destination, inbound direction and ringing/in-progress status. Failed/late lookups refuse the call. Subsequent turns use the same per-session capabilities, decision routing and explicit confirmation as outbound. Keep outbound opt-in false for inbound-only testing. This laptop/tunnel arrangement is not persistent deployment.

**Current approved presentation settings:** `VOICE_PROVIDER_MODE=trial-demo`, `VOICE_TRIAL_DEMO_ACKNOWLEDGED=true`, `VOICE_DEMO_SKIP_CHALLENGE=true`, and `VOICE_CALL_SECONDS=600`. Outbound calls to the enrolled handset skip the Telegram code and begin with “say ready.” This trusts whoever answers that handset; it is demo access, not verified personal identity. Telegram remains the evidence surface. Exact spoken read-back and confirmation remain mandatory. The local session deadline and callback lifetime are ten minutes from session/link creation, rather than three minutes; ringing consumes session time. Twilio's documented trial ceiling is also ten minutes, but this mode cannot remotely enforce hangup. Outbound opt-in remains false and handset testing remains paused. The ordinary code-authenticated settings below remain available by leaving `VOICE_DEMO_SKIP_CHALLENGE=false`.

V1 supports one enrolled handset, authenticated pending-work queries and complete spoken CHOICE decisions against the v2.1.2 simulator. Numeric free-form input and multiple decisions in one call remain outside this milestone. Every CHOICE conservatively requires evidence inspection. No LLM is called by this deterministic voice path. V2 real-backend activation remains disabled: startup requires a loopback service whose demo config reports `synthetic` and contract `2.1.2`.

1. Finish the existing Telegram setup below. Keep the single Hermes gateway and enrolled actor/chat mapping. Stop the previous interaction owner before replacing it; do not run two notifiers.
2. Copy `voice.env.example` to ignored `.local/voice.env`. Fill credentials locally, with a distinct random `VOICE_BACKEND_TOKEN` of at least 32 characters, the existing Twilio account/number, the verified handset, and the existing Telegram actor. Leave `optedIn=false` until the supervised outbound test is ready. Keep secrets out of source control, logs and `VITE_*` settings.
Strict trial mode (`VOICE_PROVIDER_MODE=trial`) omits restricted creation fields but still requires a duration update before authentication; this account returned 404 for that update. Standard accounts use `standard`. For the explicitly approved, supervised synthetic outbound demo, use `trial-demo` and `VOICE_TRIAL_DEMO_ACKNOWLEDGED=true`. This alternative authenticates each start/turn URL with an expiring HMAC capability, binds it to a durably created outbound call, and still requires the enrolled actor's Telegram challenge before disclosure or decisions. It does not establish Twilio identity or sign the request body. Treat the entire URL as a bearer secret; disable ngrok inspection (`--inspect=false`), never print/share callback URLs, and use no participant or production data. Status callbacks still require Twilio signatures. Invalid supplied signatures are rejected rather than falling back to capabilities. Inbound calling is unavailable in this demo mode.

3. Back up the current `.local/f2-simulator.json` and `.local/f2-state.json`. Start `npm run dev:v1`, the existing `python scripts/f2_setup.py gateway`, and `npm run start:v1`. The first two can start before the voice service. Both Node services load `.local/f2.env` followed by `.local/voice.env`; the gateway uses the existing profile. Current demo-safe synthetic cases migrate from 2.1.1 to 2.1.2 at restore without changing history, receipt provenance, accepted work or machine assessments. EVAL snapshots are not migrated.
4. Publish **only port 5177** through the existing authenticated ngrok setup. Set `VOICE_PUBLIC_URL` to its exact HTTPS origin before starting the voice service. Never tunnel the private `/update` endpoint, Hermes bridge or backend. Strict-mode inbound settings use POST `/voice/start` and POST `/voice/status`. Outbound calls set their own callback URLs. Requests need either a valid provider signature or the explicit trial-demo capability described above; there is no unrestricted unsigned route.
5. Strict modes enforce the configured 30–180-second cap at the provider and locally. **Trial-demo enforces only the application decision deadline** and emits Hangup on a completed/deferred journey; it cannot remotely cut off the phone at that deadline. The user approved a 180-second decision window and manual hangup, with Twilio's documented 600-second trial ceiling as emergency backup. That ceiling has not been measured locally. Quiet hours default to 22:00–08:00 Asia/Kuala_Lumpur. An outbound call also requires explicit opt-in, Telegram notifications enabled, a configured `outboundCaseId`, and a current OPEN CHOICE review. The notifier records one durable attempt per actor/review/run; a timeout is never automatically redialed. Phone activity never marks `notified_at`.

### Short handset acceptance gate

Run `npm run test:voice` and `npm run check` first. Verify the existing trial quota, expiry, verified handset and selected voice before dialing; never upgrade or purchase a number as a fallback. Use synthetic fixtures only. Recording is off; this application stores interpreted proposals and routing/audit metadata, not raw speech. Twilio processes the speech and may retain provider request/call logs under its account settings; participant content is not authorized by this test.

For one supervised outbound attempt, use the open `demo_candidate-choice` fixture, enable its configured opt-in, and keep Telegram ready with `/voice ` typed. The call speaks six separate digits with pauses, repeats them once, and allows a 20-second typing pause. Send `/voice 123456` using that call's actual code, then say **ready** after the bot approves it. Say **repeat** if needed. Select the offered review number. Inspect the delivered original documents and actual locators, then say **I have checked**. Choose the supported option using **Alpha** for A, **Bravo** for B, or **Charlie** for C. Listen to the exact read-back and say **yes** only if correct. Say **status** after acceptance to hear the fresh simulator result. The call completes the decision; no Telegram decision button is required. “No,” “not now,” ambiguity or silence cannot confirm a proposal.

Capture the call direction/duration, callback authentication mode (provider signature versus trial-demo capability), authenticated actor, review/run IDs, exact accepted `channel=VOICE` decision, fresh workflow outcome, unchanged machine assessment and before/after trial usage when available. Record the user's listening result. Do not claim V1 complete until this real-handset journey passes. A capability must not be reported as a provider signature. The earlier unauthenticated spike is not a substitute for this journey. Stop the temporary tunnel/services afterward and turn outbound opt-in off. Preserve durable state for audit; do not erase attempt records to cause an automatic retry.

The implementation follows Twilio's [webhook signature guidance](https://www.twilio.com/docs/usage/webhooks/webhooks-security), [speech Gather](https://www.twilio.com/docs/voice/twiml/gather) and [call duration controls](https://www.twilio.com/docs/voice/api/call-resource). Actual account restrictions still require handset verification.
