...
- [ ] Actual authorized Telegram handshake, source delivery, decision, dashboard result and gateway restart.
- [x] F0/F1 regressions and unchanged authoritative documents.
- [ ] Real backend handshake (separate external gate).

Checked items denote simulator/native SDK verification, not live Telegram delivery. See F2-verification.md for evidence and the pinned gateway cold-start limitation.
