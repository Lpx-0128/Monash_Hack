...
**V1 acceptance update:** The complete outbound handset journey passed on 20 September 2026 in the user-approved supervised synthetic-demo mode (no Telegram challenge, 600-second session window). One VOICE decision was accepted, the simulator completed, and the user heard the result before hangup. See [verification](./verification.md). V2 real-backend integration and V3 deployment/rehearsal remain pending.  
...
**V1 scope clarification — 20 September 2026:** V1 includes the complete authenticated voice decision journey against the v2.1.2 **simulator**: current inbox, explicit review selection, Telegram evidence where needed, spoken choice, exact read-back and explicit confirmation, structured `DecisionRequest` validation, simulated acceptance/resumption, and reporting the observed result. Telegram decision buttons are not required. Run simulated-call tests first, then one short real-handset acceptance test before declaring V1 complete. V2 repeats this journey against the real backend; V3 covers reliable deployment, rehearsal and submission. Simulator success does not establish real-backend readiness or team sign-off. This clarification supersedes the earlier read-only V1 boundary.
...
**Approved supervised trial exception — 20 September 2026:** For this synthetic V1 outbound demo only, the user approved a 180-second application decision deadline with manual hangup and Twilio's documented 600-second trial cutoff as emergency backup. Explicit `trial-demo` mode uses expiring per-step bearer capabilities and enrolled Telegram challenge authentication because the trial proxy omitted the initial provider signature. Capabilities do not establish provider identity or sign request bodies; status callbacks still require signatures. Keep tunnel inspection disabled and callback URLs secret. This exception does not authorize participant/production data, V2, or a claim of a provider-enforced three-minute cutoff. The first repaired handset test reached the authentication prompt but ended before the user could type the code; the longer authentication pause is automated-tested and awaits handset acceptance.

## 1. Recommendation and evidence

**V1 status correction:** The completed outbound handset test does not by itself complete V1's bidirectional scope. Inbound calls must also use the shared simulator review/confirmation path and pass a separate handset test. The implementation now includes a protected incoming-call bootstrap with provider-record verification, but handset acceptance must be recorded separately before claiming both directions work.

**Latest supervised-demo amendment:** The user subsequently approved skipping the Telegram challenge for explicitly initiated calls to the enrolled handset and replacing the three-minute session limit with 600 seconds. This applies only to `trial-demo` against the synthetic simulator. Record access as `demo-handset`, not Telegram-verified identity. Preserve protected callbacks, evidence inspection, exact read-back and explicit confirmation. This supersedes the prior 180-second/code requirement only for this supervised demo and does not authorize real-backend use or additional test calls. Handset testing remains paused.
...
| V1 — complete simulator voice journey | Durable routing, authenticated sessions, current task inbox, provider callback validation, bounded calls, Telegram evidence, exact spoken confirmation and v2.1.2 simulator decision acceptance/resumption; simulated-call tests pass, then one short real-handset acceptance test; existing MUST flows still pass. |
