...
**Local simulator adoption:** The PRD 3 V1 scope clarification authorizes complete VOICE decisions in the local synthetic simulator using this v2.1.2 contract. Shared validators, simulator output, channel persistence and local consumers adopt v2.1.2 together. This does not authorize real-backend activation or imply A/B/C deployment approval. V1 requires simulated-call checks and one short real-handset acceptance test; V2 retains the real-backend gates in §7.3.
