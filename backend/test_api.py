...
from backend.schemas import Case, WorkflowStatus, EmailCategory


def poll_case(client: TestClient, case_id: str, expected_status: str, timeout: int = 15) -> dict:
...
def run_tests():
    with TestClient(app, raise_server_exceptions=True) as client:
        ...
        data = poll_case(client, case_id, "AWAITING_HUMAN", timeout=15)
        print(f"  Machine status: {data['machine_assessment']['status']}")
        assert len(data["fields"]) == 7, f"Expected 7 fields, got {len(data['fields'])}"
        ...
        reviews = r.json()
        matching = [rev for rev in reviews if rev["review"]["case_id"] == case_id]
        assert len(matching) == 1, f"Expected 1 open review for case, got {len(matching)}"
        review_id = matching[0]["review"]["review_id"]
        run_id = matching[0]["review"]["run_id"]
        ...
        final = poll_case(client, case_id, "COMPLETED", timeout=15)
        ...
        print("  Review run_id matches new run [OK]")
        ...
        print("  All enum keys present [OK]")
