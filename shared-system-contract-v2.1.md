...
- The attached preliminary judging rubric. Earlier conversation findings report the participant vocabulary and submission shape adopted below. The participant bundle was not independently re-executed during this editorial update.
...
export interface Case {
  ...
  run: RunRef;
  email: {
    email_id: string; from: string; subject: string; received_at: string;
...
|---|---|---|
| Vocabulary, seven fields, output shape | Adopt the participant-facing findings recorded in the prior conversation; validate adapter against the pinned participant schema in B0 | B |
...
The prior conversation reports 520 emails and 250 attachments across TXT/PDF/DOCX/XLSX. Treat these as planning observations; compute the actual required ID set and format inventory from the authorized input at implementation time.
...
This is a full valid processing-state fixture. It deliberately has no classification or assessment yet; it is not an example of a completed BL comparison.
